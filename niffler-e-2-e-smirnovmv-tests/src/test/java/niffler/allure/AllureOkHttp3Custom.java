 ...
 */
public class AllureOkHttp3Custom implements Interceptor {

    private String requestTemplatePath = "http-request.ftl";
    private String responseTemplatePath = "http-response.ftl";

    public AllureOkHttp3Custom setRequestTemplate(final String templatePath) {
        this.requestTemplatePath = templatePath;
        return this;
    }

    public AllureOkHttp3Custom setResponseTemplate(final String templatePath) {
     ...
     */
    @Deprecated
    public AllureOkHttp3Custom withRequestTemplate(final String templatePath) {
     ...
     */
    @Deprecated
    public AllureOkHttp3Custom withResponseTemplate(final String templatePath) {
    ...
    public Response intercept(final Chain chain) throws IOException {
        ...
        final HttpRequestAttachment.Builder requestAttachmentBuilder = HttpRequestAttachment.Builder
                .create("Request " + request.method(), requestUrl)
                .setMethod(request.method())
                .setHeaders(toMapConverter(request.headers().toMultimap()));

        final RequestBody requestBody = request.body();
        if (Objects.nonNull(requestBody)) {
            if (requestBody.contentType() != null && "json".equals(requestBody.contentType().subtype())) {
                var requestBodyAsString = prettyPrint(readRequestBody(requestBody));
                requestAttachmentBuilder.setBody(requestBodyAsString);
            } else requestAttachmentBuilder.setBody(readRequestBody(requestBody));
        }
        ...
        if (Objects.nonNull(responseBody)) {
            ...
            responseAttachmentBuilder.setBody(prettyPrint(new String(bytes, StandardCharsets.UTF_8)));
